 ...
 *   found.
 */
let formatModel = (model: string): string => {
 ...
 */
let getFixText = (prefix: string, config: ReporterConfig): string => {
  let result = prefix
 ...
 *   prefix.
 */
let formatFilePath = (filePath: string): string => {
  let cwd = process.cwd()
 ...
 */
let buildCheckRows = (check: TokenCheckResult): TableRow[] => {
  let rows: TableRow[] = []
 ...
 */
let formatStatusMessage = (check: TokenCheckResult): string[] => {
  let messages: string[] = []
 ...
 */
let formatTable = (rows: TableRow[], check: TokenCheckResult): string[] => {
 ...
 */
let createJsonReporter = (process: NodeJS.Process): Reporter => {
  let print = (data: unknown): void => {
 ...
 *   consumption.
 */
let createHumanReporter = (
  process: NodeJS.Process,
  isSilentMode = false,
): Reporter => {
  let output = ''

  let print = (...lines: string[]): void => {
