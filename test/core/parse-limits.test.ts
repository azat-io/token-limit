...
describe('parseLimit', () => {
  describe('string and number inputs', () => {
    ...
    it('should parse model names as token limits', () => {
      ...
      expect(parseLimit('gpt-4')).toEqual({ tokens: 8192 })
      expect(parseLimit('GPT-4')).toEqual({ tokens: 8192 })
    })
    ...
    it('should throw error for invalid token format', () => {
      ...
      expect(() => parseLimit('1x')).toThrow('Invalid token limit format: "1x"')
  ...
  describe('object inputs', () => {
    it('should parse token limit objects', () => {
      ...
      expect(parseLimit({ tokens: 'gpt-4' })).toEqual({ tokens: 8192 })
    })
